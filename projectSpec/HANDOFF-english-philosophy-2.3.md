...
- **LLM call needs a per-request timeout.** Without it, a hung network/proxy call blocks the worker forever (order stuck "generating", retries never fire). Add a configurable timeout (~180s) so a hung call aborts → retries → fails cleanly, from day one. (DrawReport uses Anthropic Claude via `pipeline/llm.py` — apply the same per-call timeout there.)
...
> **DrawReport note (added on intake):** DrawReport is already built on the OLD "skills-only" philosophy
> (M0–M9, VERSION ~0.015). Adopting 2.3 here is a **rework of the existing pipeline + landing**, not a
> green-field build: rewrite `pipeline/prompt.py` (`PROMPTS["en"]`), `pipeline/schema.py` (new fields +
> the 7 new directions), `pipeline/lint.py` (frame-check model), `templates/report.html`,
> `templates/landing.html`/routes, and add `config/report_texts.json`. The RU reference at
> `C:\projects\GolosRisunka` now contains the v4.0 implementation — mirror its structure.
> **OWNER CALIBRATION (overrides §2's "dial zone-3 down"):** KEEP the Russian depth in English; safety =
> the airtight safe frame + prominent disclaimers that all interpretation is a SUGGESTION/HYPOTHESIS
